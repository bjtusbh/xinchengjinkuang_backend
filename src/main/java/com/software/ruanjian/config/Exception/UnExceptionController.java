package com.software.ruanjian.config.Exception;
...
public class UnExceptionController {

    /* 捕获shiro 异常 */
    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    @ExceptionHandler(ShiroException.class)
    public ResponseData handle401(ShiroException e) {
        return new ResponseData(ExceptionMsg.UnAuthorization, e.getMessage());
    }
    ...
    public ResponseData globalException(HttpServletRequest request, Throwable ex) {
        return new ResponseData(getStatus(request).toString(), ex.getMessage(), null);
    }
